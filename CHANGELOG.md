...
- Added `MultiResolutionModel`, a residual-correction forecasting model combining a low-frequency `base_model` (trained on the target aggregated to `base_freq`) with a high-frequency `residual_model` correcting the base forecast at the native resolution. Useful for horizons longer than what the base model captures well on its own. [#3174](https://github.com/unit8co/darts/pull/3174) by [remyx-ai[bot]](https://github.com/remyx-ai).
